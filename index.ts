...
class Bulvytes extends Produktas {
  ...
  constructor(kiekis: number, tipas: BulvytesTipas = BulvytesTipas.Lazdeles) {
...
class Padazas extends Produktas {
  constructor(public readonly tipas: PadazoTipas, pavadinimas: string) {
...
function kopijuotiProdukta(barcode: number): void {
  ...
  const produktas = produktai.find(
    (produktas) => produktas.barcode === barcode
  );

  if (!produktas) throw new Error('Produktas nerastas!');
